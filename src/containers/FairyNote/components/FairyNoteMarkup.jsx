...
const FairyNoteMarkup = props => {
    ...
    useEffect(() => {
        ...
        document.addEventListener("dragover", ( event ) => {
            ...
            if (dragX !== event.pageX){
                ...
                let newtime = progressBar.attributes.getNamedItem('aria-valuemax').value * offset / pRect.width
    ...
    let handleDragEnd = index => e => {
        ...
        offset = Math.max(0,offset)
        offset = Math.min(pRect.width,offset)
        let newtime = totalTime * offset / pRect.width
        itemUpdate({
            index: index,
            item: {
                timestamp: newtime
        ...
        style={{transform: "translateX(-0.5em)", left: item.timestamp*100/totalTime + "%" ,bottom: "-2px",position:"absolute"}}>
            <Popover content={content(item, index)} size="small" title="FairyNote2" placement="topRight" overlayStyle={{zIndex: 6000}}>
                <div size="small" type="link" onClick={() => {
                    ...
                    {
                        item.active ? (<DownSquareFilled style={{color:"rgba(255, 255, 0, 0.8)"}} />) : (<CaretDownFilled style={{color:"rgba(48, 200, 255, 0.8)"}}/>)
                    }
                </div>
            </Popover>
        </div>
    ...
    return (
        <Tooltip placement="topRight" title={t("progresshint")}  overlayStyle={{whiteSpace: "pre-wrap", textAlign:"right"}}>
